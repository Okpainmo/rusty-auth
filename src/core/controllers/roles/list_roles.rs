use crate::AppState;
use crate::core::services::role::find_role::find_all_roles;
use crate::core::services::sub_session::create_sub_session::{
    CreateSubSession, create_sub_session,
};
use crate::core::structs::role::Role;
use crate::middlewares::sessions_middleware::SessionsMiddlewareOutput;
use axum::extract::{Extension, State};
use axum::{
    Json,
    http::{Method, StatusCode, Uri},
    response::IntoResponse,
};
use serde::Serialize;
use tracing::error;

#[derive(Debug, Serialize)]
pub struct ListRolesResponseCore {
    data: Vec<Role>,
    session_id: String,
    access_token: String,
    refresh_token: String,
}

#[derive(Debug, Serialize)]
pub struct ListRolesResponse {
    response_message: String,
    response: Option<ListRolesResponseCore>,
    error: Option<String>,
}

pub async fn list_roles(
    State(state): State<AppState>,
    Extension(session_output): Extension<SessionsMiddlewareOutput>,
    method: Method,
    uri: Uri,
) -> impl IntoResponse {
    match find_all_roles(&state.db).await {
        Ok(roles) => {
            if let Err(e) = create_sub_session(
                &state.db,
                CreateSubSession {
                    session_id: session_output.session.id,
                    user_id: session_output.user.id,
                    activity_type: "list_roles".to_string(),
                    activity_description: Some("List roles endpoint accessed".to_string()),
                    ip_address: None,
                    user_agent: None,
                    request_method: method.as_str().to_string(),
                    request_path: uri.path().to_string(),
                },
            )
            .await
            {
                error!("FAILED TO CREATE LIST ROLES SUB-SESSION: {}", e);
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(ListRolesResponse {
                        response_message: "Failed to list roles".to_string(),
                        response: None,
                        error: Some(e.to_string()),
                    }),
                );
            }

            (
                StatusCode::OK,
                Json(ListRolesResponse {
                    response_message: "Roles fetched successfully".to_string(),
                    response: Some(ListRolesResponseCore {
                        data: roles,
                        session_id: session_output.session.id.to_string(),
                        access_token: session_output.access_token,
                        refresh_token: session_output.session_token,
                    }),
                    error: None,
                }),
            )
        }
        Err(e) => {
            error!("FAILED TO LIST ROLES: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ListRolesResponse {
                    response_message: "Failed to list roles".to_string(),
                    response: None,
                    error: Some(e.to_string()),
                }),
            )
        }
    }
}
