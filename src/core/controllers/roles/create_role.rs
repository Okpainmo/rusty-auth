use crate::AppState;
use crate::core::services::role::create_role::{CreateRole, create_role};
use crate::core::services::sub_session::create_sub_session::{
    CreateSubSession, create_sub_session,
};
use crate::core::structs::role::Role;
use crate::middlewares::sessions_middleware::SessionsMiddlewareOutput;
use axum::extract::{Extension, State};
use axum::{
    Json,
    http::{Method, StatusCode, Uri},
    response::IntoResponse,
};
use serde::{Deserialize, Serialize};
use tracing::error;

#[derive(Debug, Deserialize)]
pub struct CreateRoleRequest {
    name: String,
    description: Option<String>,
}

#[derive(Debug, Serialize)]
pub struct CreateRoleResponseCore {
    data: Role,
    session_id: String,
    access_token: String,
    refresh_token: String,
}

#[derive(Debug, Serialize)]
pub struct CreateRoleResponse {
    response_message: String,
    response: Option<CreateRoleResponseCore>,
    error: Option<String>,
}

pub async fn create_role_controller(
    State(state): State<AppState>,
    Extension(session_output): Extension<SessionsMiddlewareOutput>,
    method: Method,
    uri: Uri,
    Json(payload): Json<CreateRoleRequest>,
) -> impl IntoResponse {
    match create_role(
        &state.db,
        CreateRole {
            name: payload.name,
            description: payload.description,
        },
    )
    .await
    {
        Ok(role) => {
            if let Err(e) = create_sub_session(
                &state.db,
                CreateSubSession {
                    session_id: session_output.session.id,
                    user_id: session_output.user.id,
                    activity_type: "create_role".to_string(),
                    activity_description: Some("Create role endpoint accessed".to_string()),
                    ip_address: None,
                    user_agent: None,
                    request_method: method.as_str().to_string(),
                    request_path: uri.path().to_string(),
                },
            )
            .await
            {
                error!("FAILED TO CREATE ROLE SUB-SESSION: {}", e);
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(CreateRoleResponse {
                        response_message: "Failed to create role".to_string(),
                        response: None,
                        error: Some(e.to_string()),
                    }),
                );
            }

            (
                StatusCode::CREATED,
                Json(CreateRoleResponse {
                    response_message: "Role created successfully".to_string(),
                    response: Some(CreateRoleResponseCore {
                        data: role,
                        session_id: session_output.session.id.to_string(),
                        access_token: session_output.access_token,
                        refresh_token: session_output.session_token,
                    }),
                    error: None,
                }),
            )
        }
        Err(e) => {
            error!("FAILED TO CREATE ROLE: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(CreateRoleResponse {
                    response_message: "Failed to create role".to_string(),
                    response: None,
                    error: Some(e.to_string()),
                }),
            )
        }
    }
}
